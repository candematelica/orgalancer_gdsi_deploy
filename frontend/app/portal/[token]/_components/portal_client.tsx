...
export default function PortalClient({ project, tasks, freelancerName }: Props) {
    ...
    const inProgressTasks = tasks.filter((t) => t.status === "in_progress" || t.status === "En Progreso").length;
    const pendingTasks = tasks.filter(
        (t) => t.status !== "completed" && t.status !== "in_progress" && t.status !== "Completada" && t.status !== "En Progreso"
    ).length;
                            ...
                            <div className="divide-y divide-white/5">
                                {tasks.map((task) => (
                                    ...
                                    >
                                        ...
                                        <span
                                            className={`text-xs px-2 py-0.5 rounded-full ${task.status === "Completada"
