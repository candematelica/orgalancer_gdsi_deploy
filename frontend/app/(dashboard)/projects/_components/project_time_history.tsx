...
import { useProjectTimeHistory } from "../_hooks/use_project_time_history";
...
export default function ProjectTimeHistory({ projectId, refreshKey }: ProjectTimeHistoryProps) {
  const {
  ...
  } = useProjectTimeHistory(projectId, refreshKey);
  ...
  return (
    <>
      <div className="space-y-4">
        {/* Resumen */}
        <div className="bg-gradient-to-r from-blue-50 to-cyan-50 border border-blue-200 rounded-2xl p-5 flex items-center justify-between">
          <div>
        ...
        {entries.length === 0 ? (
          ...
          <div className="space-y-2">
            {entries.map((entry) => (
              <div key={entry.id}
                className="bg-white border border-blue-100 rounded-xl px-4 py-3 flex items-center gap-3 group hover:shadow-sm transition-shadow"
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {entry.task_name && (
                    <p className="text-xs text-blue-500 mt-0.5">Tarea: {entry.task_name}</p>
                  )}
          ...
          <div className="bg-white rounded-2xl p-6 max-w-sm w-full shadow-xl">
            ...
            <div className="space-y-3">
              <div>
                ...
                <input type="date" value={editDate} max={new Date().toISOString().split("T")[0]}
                  ...
                  className="w-full px-3 py-2 border border-blue-200 rounded-xl text-sm focus:outline-none focus:border-blue-400" />
              </div>
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <input type="number" min="0" max="23" value={editHours}
                    ...
                    className="w-full px-3 py-2 border border-blue-200 rounded-xl text-sm text-center focus:outline-none focus:border-blue-400" />
                </div>
                <div>
              ...
              <div>
                ...
                <textarea value={editDesc} onChange={(e) => setEditDesc(e.target.value)} rows={2}
                  className="w-full px-3 py-2 border border-blue-200 rounded-xl text-sm resize-none focus:outline-none focus:border-blue-400" />
              </div>
            </div>
            <div className="flex justify-end gap-2 mt-4">
              ...
              <button onClick={saveEdit} disabled={saving}
                className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-gradient-to-r from-blue-600 to-cyan-600 text-white text-sm font-medium disabled:opacity-60">
